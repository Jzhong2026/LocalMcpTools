...
@pytest.fixture
def workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):  # type: ignore[no-untyped-def]
...
def test_start_is_denied_for_observe(workspace) -> None:  # type: ignore[no-untyped-def]
    database, item = workspace
    wrapper = ToolExecutionService(audit_path=database).register(
        "process.start_dev_server", process_start_dev_server,
...
def test_unknown_preset_is_reported_after_profile_gate(workspace) -> None:  # type: ignore[no-untyped-def]
    database, item = workspace
    with db.connection(database) as conn:
...
def test_approved_start_returns_managed_contract(workspace, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    database, item = workspace
    with db.connection(database) as conn:
        conn.execute("UPDATE workspaces SET profile='managed_process' WHERE id=?", (item.id,))
    fake = ManagedProcess(
        id="mp-contract", workspace_id=item.id, preset="node-vite",
        command="npx vite", cwd=item.canonical_root, pid=4242,
        log_handle="art://2026-08-07/calls/fake.log", port=None,
        started_at=1, persistent=False, status="running", exit_code=None,
        finished_at=None,
    )
    monkeypatch.setattr(
    ...
    response = wrapper(
        workspace_id=item.id, preset="node-vite", args=[], approval_id=approval_id
    )
    assert response["ok"] is True
    assert response["data"] == {
        "id": "mp-contract", "pid": 4242, "command_resolved": "npx vite",
        "log_handle": fake.log_handle, "port": None,
    }
    with db.connection(database) as conn:
        audit = conn.execute(
...
def test_long_shell_timeout_redirects_to_managed_process(workspace) -> None:  # type: ignore[no-untyped-def]
    database, item = workspace
    with db.connection(database) as conn:
        conn.execute("UPDATE workspaces SET profile='workspace_exec' WHERE id=?", (item.id,))
    wrapper = ToolExecutionService(audit_path=database).register(
        "shell.run_command", shell_run_command,
...
def test_missing_port_is_typed(monkeypatch) -> None:  # type: ignore[no-untyped-def]
