...
def test_digest_is_stable_across_key_order_and_ignores_retry_handle() -> None:
    one = digest_for("shell.run_command", {"cmd": "echo hi", "timeout_ms": 1}, "ws", "workspace_exec")
    two = digest_for("shell.run_command", {"timeout_ms": 1, "approval_id": "a", "cmd": "echo hi"}, "ws", "workspace_exec")
    assert one == two


def test_approved_matching_action_is_consumed_once(database: Path) -> None:
    with db.connection(database) as conn:
        item = request("ws", "workspace_exec:shell.run_command", {"cmd": "echo hi"}, profile="workspace_exec", conn=conn)
        ...
        row = conn.execute("SELECT status, consumed_at FROM approvals WHERE id = ?", (item.id,)).fetchone()
...
def test_digest_mismatch_does_not_consume_approval(database: Path) -> None:
    with db.connection(database) as conn:
        ...
        status = conn.execute("SELECT status FROM approvals WHERE id = ?", (item.id,)).fetchone()["status"]
    assert status == "approved"


def test_expired_approval_is_marked_expired(database: Path) -> None:
    with db.connection(database) as conn:
        item = request("ws", "workspace_exec:shell.run_command", {"cmd": "echo hi"}, profile="workspace_exec", ttl_ms=1, conn=conn)
        time.sleep(0.01)
